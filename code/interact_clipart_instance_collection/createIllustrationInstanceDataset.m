...
if ( 1 )
    ...
    allDataCell = cell(numUnqLabels, 2);
    
    for i = 1:numUnqLabels
        
        curLabel = unqLabels(i);
        
        curVerb = verbsName{curLabel};
        allDataCell{i, 1} = curVerb;
        
        
        curImgs = find(imgLabels == curLabel);
        
        curImgDataCell = cell(length(curImgs), 2);
        for j = 1:length(curImgs)
            imgNum = curImgs(j);
            curClipartImgLabels = find(imgNum == clipartImgLabels);
            
            curImgFilename = imgFilenames{imgNum};
            curClipDataCell = cell(length(curClipartImgLabels), 2);
            
            curImgDataCell{j, 1} = curImgFilename;
            for k = 1:length(curClipartImgLabels)
                ...
                curClipDataCell{k, 1} = imgName;
                curClipDataCell{k, 2} = curP1;
...
%                 labels = [labels; verbLabel];
                ...
                if ( fileWrite )
%                     copyfile(fullfile(options.foldersLocal{options.idxINTERACTColl}{options.idxDataOutput}, 'found_imgs', curImgFilename), fullfile(standaloneDatasetImgsFolder, curImgFilename));
%                     copyfile(fullfile(options.foldersLocal{options.idxINTERACTClipInstColl}{options.idxDataOutput}, 'rendered_illustrations', imgName), fullfile(standaloneDatasetImgsFolder, imgName));
%                     
                    fprintf(labelsAndFilesFID, '%02d;%s;%s;%02d\n', verbLabel, imgName, curImgFilename, curP1);
                    fprintf(posesFID, '%s\n', poseDataStr(1:end-1));
            ...
            curImgDataCell{j, 2} = curClipDataCell;
        end
        allDataCell{i, 2} = curImgDataCell;
    end
...
CreateHTMLPagesByVerbAndImg(allDataCell, ...
...
zipCmdStr = sprintf('zip %s %s %s %s %s %s %s %s', ...
    ...
    'README_interact_illustration_instance_dataset.txt');
system(zipCmdStr);
