...
function tasks = ProcessUserScenes(options, taskStruct, resHeaders, resData)
    ...
    headersWithDiffs = strfind( resHeaders, 'diff');
    diffsIdxs = find(not(cellfun('isempty', headersWithDiffs)));
    ...
    for idxStruct = 1:size(resData, 1)
        ...
        for idxData = 1:length(posesIdxs);
            ...
            filename = inputParts{idxData};
            diff = str2double(resData{idxStruct, diffsIdxs(idxData)});

            % All below adheres to the answer string provided by the AMT
            % interface HTML file.
            newScene.img = filename;
            newScene.diff = diff;
