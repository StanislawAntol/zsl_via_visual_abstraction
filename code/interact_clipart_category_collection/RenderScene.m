...
function finalImg = RenderScene(options, scene)
    ...
    clipartDataStruct = options.clipartInterfaceGlobals;
    
    %% Load background image
    bgImg = imread([clipartDataStruct.imgDirectory clipartDataStruct.background]);
    bgImg = bgImg(1:clipartDataStruct.height,1:clipartDataStruct.width, :);

    %% Display background image
    h = imshow(bgImg);
    hold on;
    set(h,'Tag','export_fig_native');
    title(scene.sent, 'FontSize', 24);
    ...
    processPerson(clipartDataStruct, hair, woman, ...
    ...
    finalImg = finalImg((1:clipartDataStruct.height)+1, (1:clipartDataStruct.width)+1, :);

end

% % Helper function to process a person
% % INPUT
% % -clipartDataStruct: the struct from getDollGlobals (containing relevant
% %                  data).
...
function processPerson(clipartDataStruct, hair, woman, xPose, yPose, rPose, expr, flip)

    numParts = numel(clipartDataStruct.renderOrder);
    xScale = clipartDataStruct.personScale;
    yScale = clipartDataStruct.personScale;

    for i = 1:numParts

        idx = clipartDataStruct.renderOrder(i);

        %% Loads the correct image for the part
        if (idx ~= clipartDataStruct.headIdx)
            if (woman == 0)
                if ( hair == 0 ) % Yellow/blonde
                    [part, ~, alph] = imread([clipartDataStruct.imgDirectory, clipartDataStruct.manYPoseFiles{idx}] );
                else % Brown
                    [part, ~, alph] = imread([clipartDataStruct.imgDirectory, clipartDataStruct.manBPoseFiles{idx}] );
                end
            else % (woman == 1)
                if ( hair == 0 ) % Yellow/blonde
                    [part, ~, alph] = imread([clipartDataStruct.imgDirectory, clipartDataStruct.womanYPoseFiles{idx}] );
                else % Brown
                    [part, ~, alph] = imread([clipartDataStruct.imgDirectory, clipartDataStruct.womanBPoseFiles{idx}] );
                end
            end
        else
            if (woman == 0)
                [part, ~, alph] = imread([clipartDataStruct.imgDirectory, clipartDataStruct.manExpFiles{expr}] );
            else % (woman == 1)
                [part, ~, alph] = imread([clipartDataStruct.imgDirectory, clipartDataStruct.womanExpFiles{expr}] );
            end        
        end

        %% X1 contains the local offset of the part
        if (woman == 0)
            X1 = [-clipartDataStruct.x1PoseMan(idx);
                  -clipartDataStruct.y1PoseMan(idx);];
        else % (woman == 1)
            X1 = [-clipartDataStruct.x1PoseWoman(idx);
                  -clipartDataStruct.y1PoseWoman(idx);];        
        end
