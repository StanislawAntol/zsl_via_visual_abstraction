...
function finalImg = RenderScene(options, scene)
    ...
    bgImg = imread(fullfile(dollDataStruct.imgDirectory, dollDataStruct.background));
    ...
    %% Process and display the clipart personA first
        ...
        if (idx ~= dollDataStruct.headIdx)
            if (woman == 0)
                if ( hair == 0 ) % Yellow/blonde
                    [part, ~, alph] = imread(fullfile(dollDataStruct.imgDirectory, dollDataStruct.manYPoseFiles{idx}) );
                else % Brown
                    [part, ~, alph] = imread(fullfile(dollDataStruct.imgDirectory, dollDataStruct.manBPoseFiles{idx}) );
                end
            else % (woman == 1)
                if ( hair == 0 ) % Yellow/blonde
                    [part, ~, alph] = imread(fullfile(dollDataStruct.imgDirectory, dollDataStruct.womanYPoseFiles{idx}) );
                else % Brown
                    [part, ~, alph] = imread(fullfile(dollDataStruct.imgDirectory, dollDataStruct.womanBPoseFiles{idx}) );
                end
            end
        else
            if (woman == 0)
                [part, ~, alph] = imread(fullfile(dollDataStruct.imgDirectory, dollDataStruct.manExpFiles{expr}) );
            else % (woman == 1)
                [part, ~, alph] = imread(fullfile(dollDataStruct.imgDirectory, dollDataStruct.womanExpFiles{expr}) );
            end        
        end
