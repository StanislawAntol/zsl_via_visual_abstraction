...
% function runINTERACTCategoryPartThresholdExperiment
...
allImgsWithEnoughParts = INTERACTImagesWithCertainNumberParts(options, parameters);

for partThreshIdx = 1:length(parameters.partThreshold)
    partThreshold = parameters.partThreshold(partThreshIdx);
    
    allImgsGoodJointsLogIdxs = allImgsWithEnoughParts(:, partThreshold) == 1;
    sum(allImgsGoodJointsLogIdxs)
    allImgsGoodJointsIdxs = find(allImgsGoodJointsLogIdxs);
    ...
    for nFt = 1:numFeatSets
        ...
        for nDetType = 1:numDetTypes % TODO: Fix to be variable with detection schemes
            if (nDetType == 1)
                ...
                allTestDataFeats{nDetType, 1} = allTestDataFeats{nDetType, 1}(allImgsGoodJointsIdxs, :);
                allTestDataFeats{nDetType, 2} = cell2mat(allINTERACTImgGTFeatures(featsToUse, 2)');
                allTestDataFeats{nDetType, 2} = allTestDataFeats{nDetType, 2}(allImgsGoodJointsIdxs, :);
                allTestDataLabels{nDetType}   = allINTERACTImgGTLabels(allImgsGoodJointsIdxs);
            elseif (nDetType == 2)
        ...
        if ( options.runINTERACTCatPartThresholdExpTrain ~= 0 )
                        ...
                        for randIdx = 1:parameters.numRandKs(kIdx)
                            ...
                            for nDetType = 1:numDetTypes
                                ...
                                saveString = sprintf(['save ', options.saveModelZSLCatFormatStr, '.mat models modelParams decValues1 decValues2 testLabels'], ...
                                    [options.classificationModelFileCatPart, sprintf('_%02d', partThreshold)], datasetNum, featsStr, detMethod, biasTrain, probTest, K, curSeedValForK);
                                eval(saveString);
                            end
                        end
                    end
...
if ( options.runINTERACTCatPartThresholdExpAccCalc ~= 0 )
    [accuraciesOverall, accuraciesAvgClass, averagePrecision, averagePrecisionRandom] = ComputeAllAccuraciesZSLCatPart(options, parameters, datasetNum);
    ...
    saveString = sprintf(['save ', options.saveAccuraciesZSLCatPartFormatStr, '.mat accuraciesOverall accuraciesAvgClass averagePrecision averagePrecisionRandom'], ...
...
if ( options.runINTERACTCatPartThresholdExpAccPlot ~= 0 )
    filename = sprintf([options.saveAccuraciesZSLCatPartFormatStr, '.mat'], ...
        options.classificationAccuraciesFile, datasetNum, parameters.featsStr, parameters.detMethodsStr, ...
        parameters.biasTrainStr, parameters.probTestStr, parameters.trainOnKsStr, parameters.numRandKsStr);
    load(filename);
    ClassificationCategoryAccuracyPartPlot(options, parameters, accuraciesOverall, accuraciesAvgClass, datasetNum)
end